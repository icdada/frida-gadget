namespace Frida.Gadget {
#if ANDROID
	[CCode (cname = "pvz2_get_embedded_config_json")]
	private extern unowned string pvz2_get_embedded_config_json ();

	[CCode (cname = "pvz2_get_embedded_agent_js_bytes")]
	private extern Bytes pvz2_get_embedded_agent_js_bytes ();

	[CCode (cname = "pvz2_secure_log")]
	private extern void pvz2_secure_log (string message);
#endif
#if IOS
	[CCode (cname = "pvz2_get_ios_embedded_config_json")]
	private extern unowned string pvz2_get_ios_embedded_config_json ();

	[CCode (cname = "pvz2_get_ios_embedded_agent_js_bytes")]
	private extern Bytes pvz2_get_ios_embedded_agent_js_bytes ();
#endif
	...
	private Config load_config (Location location) throws Error {
		#if IOS
			return parse_config (pvz2_get_ios_embedded_config_json ());
		#endif
...
#if ANDROID
			return parse_config (pvz2_get_embedded_config_json ());
#else
...
#if ANDROID
				pvz2_secure_log ("bytecode decrypted and authenticated");
#endif
...
#if ANDROID
				pvz2_secure_log ("QuickJS bytecode accepted");
#endif
...
#if ANDROID
				pvz2_secure_log ("agent bytecode loaded");
#endif
				yield call_init ();
			} finally {
...
#if IOS
		bytes = pvz2_get_ios_embedded_agent_js_bytes ();
		return;
#elif ANDROID
		if (Path.get_basename (filename) == "libpvz2_agent.js") {
			bytes = pvz2_get_embedded_agent_js_bytes ();
			return;
		}
